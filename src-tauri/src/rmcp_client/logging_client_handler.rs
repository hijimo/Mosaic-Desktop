...
use crate::rmcp_client::rmcp_client::SendElicitation;
...
impl ClientHandler for LoggingClientHandler {
    ...
    async fn on_progress(
        ...
        info!(
            "MCP server progress (token: {:?}, progress: {}, total: {:?})",
            params.progress_token, params.progress, params.total
        );
    }
    ...
    async fn on_logging_message(
        ...
        match level {
            ...
            | LoggingLevel::Error => {
                error!("MCP log ({:?}, {:?}): {}", level, logger, data);
            }
            LoggingLevel::Warning => {
                warn!("MCP log ({:?}, {:?}): {}", level, logger, data);
            }
            LoggingLevel::Notice | LoggingLevel::Info => {
                info!("MCP log ({:?}, {:?}): {}", level, logger, data);
            }
            LoggingLevel::Debug => {
                debug!("MCP log ({:?}, {:?}): {}", level, logger, data);
            }
        }
    }
}

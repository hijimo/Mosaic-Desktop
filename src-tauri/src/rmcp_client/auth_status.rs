...
/// Authentication status for a streamable HTTP MCP server.
...
pub async fn determine_streamable_http_auth_status(
    ...
    if has_oauth_tokens(server_name, url)? {
...
mod tests {
    ...
    #[test]
    fn discovery_paths_root() {
        let paths = discovery_paths("/");
        assert_eq!(paths, vec!["/.well-known/oauth-authorization-server"]);
    }

    #[test]
    fn discovery_paths_with_subpath() {
        let paths = discovery_paths("/mcp/v1");
        assert_eq!(paths.len(), 3);
        assert!(paths[0].contains("mcp/v1"));
    }

    #[tokio::test]
    async fn bearer_token_env_var_returns_bearer_status() {
    ...
    #[tokio::test]
    async fn authorization_header_returns_bearer_status() {
        let status = determine_streamable_http_auth_status(
            "server",
            "not-a-url",
            None,

    ...
    T: Copy + Eq,
{
    ...
    pub(crate) fn finish(&mut self) -> Vec<TaggedLineSegment<T>> {
        ...
        if !self.line_buffer.is_empty() {
            ...
            let slug = without_newline.trim_start().trim_end().to_string();
            ...
            if self.active_tag.is_none() {
                if let Some(tag) = self.match_open(&slug) {
                    ...
                    self.try_close_or_text(&slug, buffered, &mut segments);
                }
            } else {
                self.try_close_or_text(&slug, buffered, &mut segments);
            }
    ...
    fn try_close_or_text(
        &mut self,
        slug: &str,
        buffered: String,
        segments: &mut Vec<TaggedLineSegment<T>>,
    ) {
        ...
        self.push_text(buffered, segments);
    }

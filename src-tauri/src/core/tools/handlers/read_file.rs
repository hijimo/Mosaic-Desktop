...
fn default_include_header() -> bool { true }
...
impl LineRecord {
    fn is_blank(&self) -> bool { self.raw.trim().is_empty() }
    fn is_comment(&self) -> bool {
        COMMENT_PREFIXES.iter().any(|p| self.raw.trim().starts_with(p))
    }
}
...
impl ToolHandler for ReadFileHandler {
    ...
    async fn handle(&self, args: serde_json::Value) -> Result<serde_json::Value, CodexError> {
        ...
        if params.offset == 0 {
            return Err(CodexError::new(ErrorCode::InvalidInput, "offset must be a 1-indexed line number"));
        }
        if params.limit == 0 {
        ...
        if !path.is_absolute() {
            return Err(CodexError::new(ErrorCode::InvalidInput, "file_path must be an absolute path"));
        }
...
async fn read_all_lines(path: &PathBuf) -> Result<Vec<LineRecord>, CodexError> {
    ...
    let file = tokio::fs::File::open(path).await.map_err(|e| {
        CodexError::new(ErrorCode::ToolExecutionFailed, format!("failed to read file: {e}"))
    })?;
...
async fn slice_read(path: &PathBuf, offset: usize, limit: usize) -> Result<Vec<String>, CodexError> {
    use tokio::io::{AsyncBufReadExt, BufReader};
    let file = tokio::fs::File::open(path).await.map_err(|e| {
        CodexError::new(ErrorCode::ToolExecutionFailed, format!("failed to read file: {e}"))
    })?;
    ...
    if seen < offset {
        return Err(CodexError::new(ErrorCode::InvalidInput, "offset exceeds file length"));
    }
    Ok(collected)
}
...
async fn indentation_read(
    ...
    if anchor_line == 0 {
        return Err(CodexError::new(ErrorCode::InvalidInput, "anchor_line must be a 1-indexed line number"));
    }
    let guard_limit = opts.max_lines.unwrap_or(limit);
    if guard_limit == 0 {
        return Err(CodexError::new(ErrorCode::InvalidInput, "max_lines must be greater than zero"));
    }

    let all_lines = read_all_lines(path).await?;
    if all_lines.is_empty() || anchor_line > all_lines.len() {
        return Err(CodexError::new(ErrorCode::InvalidInput, "anchor_line exceeds file length"));
    }
    ...
    if final_limit == 1 {
        return Ok(vec![format!("L{}: {}", all_lines[anchor_index].number, all_lines[anchor_index].display)]);
    }
    ...
    while matches!(out.front(), Some(l) if l.raw.trim().is_empty()) { out.pop_front(); }
    while matches!(out.back(), Some(l) if l.raw.trim().is_empty()) { out.pop_back(); }

    Ok(out.into_iter().map(|r| format!("L{}: {}", r.number, r.display)).collect())
}

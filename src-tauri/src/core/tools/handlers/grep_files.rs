...
impl ToolHandler for GrepFilesHandler {
    ...
    async fn handle(&self, args: serde_json::Value) -> Result<serde_json::Value, CodexError> {
        ...
        let results = match run_rg_search(pattern, include.as_deref(), search_path, limit, &cwd).await {
            Ok(r) => r,
...
async fn run_builtin_search(
    ...
    .await
    .map_err(|e| CodexError::new(ErrorCode::ToolExecutionFailed, format!("search task failed: {e}")))?
}
...
fn builtin_grep(
    ...
    let re = regex::Regex::new(pattern).map_err(|e| {
        CodexError::new(ErrorCode::InvalidInput, format!("invalid regex: {e}"))
    })?;
    ...
    if let Some(glob) = include {
        ...
        overrides.add(glob).map_err(|e| {
            CodexError::new(ErrorCode::InvalidInput, format!("invalid glob '{glob}': {e}"))
        })?;
    ...
    for entry in builder.build() {
        ...
        if entry.file_type().map_or(true, |ft| !ft.is_file()) { continue; }
        ...
        if re.is_match(&content) {
            ...
            let display = path.strip_prefix(search_path).unwrap_or(path)

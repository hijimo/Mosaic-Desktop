...
pub fn is_known_safe_command(command: &[String]) -> bool {
    ...
    let command: Vec<String> = command.iter().map(|s| {
        if s == "zsh" { "bash".to_string() } else { s.clone() }
    }).collect();
    ...
    if let Some(script) = parse_shell_lc_script(&command) {
        ...
        if !sub_commands.is_empty() && sub_commands.iter().all(|cmd| is_safe_to_call_with_exec(cmd)) {
            return true;
        }
    }
...
fn is_safe_to_call_with_exec(command: &[String]) -> bool {
    let Some(cmd0) = command.first().map(String::as_str) else { return false };
    let Some(name) = executable_name(cmd0) else { return false };

    match name.as_str() {
        "cat" | "cd" | "cut" | "echo" | "expr" | "false" | "grep" | "head" | "id" |
        "ls" | "nl" | "paste" | "pwd" | "rev" | "seq" | "stat" | "tail" | "tr" |
        "true" | "uname" | "uniq" | "wc" | "which" | "whoami" => true,
        ...
        "sed" if command.len() <= 4
            && command.get(1).map(String::as_str) == Some("-n")
            && is_valid_sed_n_arg(command.get(2).map(String::as_str)) => true,
...
fn is_safe_git_command(command: &[String]) -> bool {
    ...
    let Some((idx, sub)) = find_git_subcommand(command) else { return false };
    ...
    let args_safe = !sub_args.iter().any(|a| {
        UNSAFE_GIT_FLAGS.contains(&a.as_str()) || a.starts_with("--output=") || a.starts_with("--exec=")
    });
...
fn find_git_subcommand(command: &[String]) -> Option<(usize, &str)> {
    ...
    const GIT_OPTS_WITH_VALUE: &[&str] = &["-C", "-c", "--config-env", "--exec-path", "--git-dir", "--namespace", "--super-prefix", "--work-tree"];
    ...
    for (idx, arg) in command.iter().enumerate().skip(1) {
        ...
        if (a.starts_with("-C") || a.starts_with("-c")) && a.len() > 2 { continue; }
        if a.starts_with("--config-env=") || a.starts_with("--exec-path=") || a.starts_with("--git-dir=")
            || a.starts_with("--namespace=") || a.starts_with("--super-prefix=") || a.starts_with("--work-tree=") { continue; }
        if GIT_OPTS_WITH_VALUE.contains(&a) { skip_next = true; continue; }
        if a == "--" || a.starts_with('-') { continue; }
        if SAFE_SUBS.contains(&a) { return Some((idx, a)); }
...
fn git_branch_is_read_only(args: &[String]) -> bool {
    ...
    for arg in args.iter().map(String::as_str) {
        match arg {
            "--list" | "-l" | "--show-current" | "-a" | "--all" | "-r" | "--remotes" | "-v" | "-vv" | "--verbose" => saw_flag = true,
...
fn is_valid_sed_n_arg(arg: Option<&str>) -> bool {
    ...
    match parts.as_slice() {
        ...
        [a, b] => !a.is_empty() && !b.is_empty() && a.chars().all(|c| c.is_ascii_digit()) && b.chars().all(|c| c.is_ascii_digit()),
        _ => false,
    }
}
...
fn parse_shell_lc_script(command: &[String]) -> Option<String> {
    ...
    if !matches!(cmd0.as_str(), "bash" | "sh") { return None; }
    // Find -c or -lc
...
fn split_shell_plain_commands(script: &str) -> Vec<Vec<String>> {
    ...
    while i < chars.len() {
        if chars[i] == '&' && i + 1 < chars.len() && chars[i + 1] == '&' {
            if !current.trim().is_empty() { result.push(shell_words_parse(current.trim())); }
            current.clear();
            i += 2; continue;
        }
        if chars[i] == '|' && i + 1 < chars.len() && chars[i + 1] == '|' {
...
impl ToolHandler for ShellHandler {
    ...
    async fn handle(&self, args: serde_json::Value) -> Result<serde_json::Value, CodexError> {
        ...
        if params.command.is_empty() {
            return Err(CodexError::new(ErrorCode::InvalidInput, "command array must not be empty"));
        }
        ...
        let child = cmd.spawn().map_err(|e| {
            CodexError::new(ErrorCode::ToolExecutionFailed, format!("failed to spawn command: {e}"))
        })?;

        let output = match tokio::time::timeout(timeout, child.wait_with_output()).await {
            Ok(Ok(output)) => output,
            Ok(Err(e)) => {
                return Err(CodexError::new(ErrorCode::ToolExecutionFailed, format!("command execution error: {e}")));
            }
            Err(_) => {

...
fn build_config_from_external(settings: &JsonValue) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_obj) = settings_obj.get("sandbox").and_then(JsonValue::as_object) {
        if let Some(true) = sandbox_obj.get("enabled").and_then(JsonValue::as_bool) {
            root.insert(
...
mod tests {
    ...
    #[test]
    fn import_home_migrates_config_skills_and_agents_md() {
        let (_root, claude_home, codex_home) = fixture_paths();
        let agents_skills = codex_home
            .parent()
            .map(|parent| parent.join(".agents").join("skills"))
        ...
        fs::write(
            claude_home.join("settings.json"),
            r#"{"model":"claude","env":{"FOO":"bar"},"sandbox":{"enabled":true}}"#,
        )
        .expect("write settings");
        fs::write(

...
/// A custom prompt discovered from the prompts directory.
...
pub fn default_prompts_dir() -> Option<PathBuf> {
    crate::config::ConfigService::find_codex_home().map(|home| home.join("prompts"))
}
...
mod tests {
    ...
    async fn parses_frontmatter_and_strips_from_body() {
        ...
        fs::write(dir.join("withmeta.md"), text).unwrap();

...
async fn op_user_turn_emits_bracket_events() {
    ...
    assert!(has_event(&events, |m| matches!(
        m,
    ...
    assert!(has_event(
        &events,
        |m| matches!(m, EventMsg::ItemStarted(ev) if matches!(&ev.item, tauri_app_lib::protocol::items::TurnItem::UserMessage(_)))
    ));
    assert!(has_event(
        &events,
        |m| matches!(m, EventMsg::ItemCompleted(ev) if matches!(&ev.item, tauri_app_lib::protocol::items::TurnItem::UserMessage(_)))
    ));
}
...
async fn op_add_to_history() {
    let events = run_ops(vec![Op::AddToHistory {
...
async fn op_get_history_entry_valid() {
    ...
    assert!(resp.is_some());
    assert!(
        resp.unwrap().entry.is_some(),
        "offset 0 should have an entry"
    );
}

#[tokio::test]
async fn op_get_history_entry_out_of_range() {
    let events = run_ops(vec![Op::GetHistoryEntryRequest {
        offset: 9999,
        log_id: 0,
    }])
    .await;
...
async fn op_set_thread_name() {
    let events = run_ops(vec![Op::SetThreadName {
        name: "My Chat".into(),
    }])
    .await;
...
async fn op_thread_rollback() {
    ...
    assert!(has_event(
        &events,
        |m| matches!(m, EventMsg::ThreadRolledBack(r) if r.num_turns == 1)
    ));
}
...
async fn op_compact_with_history() {
    ...
    assert!(
        compacted || errored,
        "compact should emit ContextCompacted or Error"
    );
}
...
async fn op_list_skills() {
    let events = run_ops(vec![Op::ListSkills {
        cwds: vec![],
        force_reload: false,
    }])
    .await;
...
async fn op_realtime_start_no_session_id() {
    let events = run_ops(vec![Op::RealtimeConversationStart(
...
async fn fuzzy_search_finds_cargo_toml() {
    ...
    assert!(matches
        .iter()
        .any(|m| m.path.to_string_lossy().contains("Cargo.toml")));
}

...
fn p01_streaming_deltas_not_persisted() {
    ...
    assert!(!is_persisted(
        &RolloutItem::EventMsg(delta.clone()),
        EventPersistenceMode::Limited
    ));
    assert!(!is_persisted(
        &RolloutItem::EventMsg(delta),
    ...
    assert!(!is_persisted(
        &RolloutItem::EventMsg(reasoning),
        EventPersistenceMode::Extended
    ));
}
...
fn p03_response_item_always_persisted() {
    assert!(is_persisted(
        &RolloutItem::ResponseItem(types::ResponseItem::Other),
        EventPersistenceMode::Limited
    ));
    assert!(is_persisted(
        &RolloutItem::ResponseItem(types::ResponseItem::Other),
...
fn p05_extended_events_only_extended() {
    let ev = EventMsg::ExecCommandEnd(ExecCommandEndEvent {
        ...
        cwd: "/tmp".into(),
        parsed_cmd: vec![],
        source: types::ExecCommandSource::Agent,
        ...
        status: types::ExecCommandStatus::Completed,
    });
    assert!(!is_persisted(
        &RolloutItem::EventMsg(ev.clone()),
        EventPersistenceMode::Limited
    ));
    assert!(is_persisted(
        &RolloutItem::EventMsg(ev),

...
async fn start_multi_round_server(rounds: Vec<String>) -> (std::net::SocketAddr, tokio::task::JoinHandle<()>) {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let handle = tokio::spawn(async move {
...
fn make_codex(base_url: &str) -> (async_channel::Sender<Submission>, async_channel::Receiver<Event>, Codex) {
    ...
    providers.insert("mock".to_string(), ModelProviderInfo {
        name: "Mock".into(),
...
async fn v01_text_response_emits_full_v2_sequence() {
    let body = format!(
        "{}{}{}",
        sse("response.output_text.delta", serde_json::json!({"type":"response.output_text.delta","delta":"Hello "})),
        sse("response.output_text.delta", serde_json::json!({"type":"response.output_text.delta","delta":"world!"})),
        sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r1","output":[],"usage":{"input_tokens":10,"output_tokens":5,"total_tokens":15}}})),
    );
    let server = MockServer::start(body).await;
    let (sq_tx, eq_rx, codex) = make_codex(&server.base_url());

    sq_tx.send(Submission { id: "t1".into(), op: user_turn("hi") }).await.unwrap();
    ...
    let deltas: Vec<&str> = events.iter().filter_map(|e| match &e.msg {
...
async fn v02_tool_call_emits_mcp_events() {
    let round1 = format!(
        "{}{}",
        sse("response.output_item.done", serde_json::json!({"type":"response.output_item.done","item":{"type":"function_call","id":"fc1","call_id":"c1","name":"shell","arguments":"{\"command\":[\"echo\",\"test\"]}"}})),
        sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r1","output":[],"usage":{"input_tokens":10,"output_tokens":5,"total_tokens":15}}})),
    );
    let round2 = format!(
        "{}{}",
        sse("response.output_text.delta", serde_json::json!({"type":"response.output_text.delta","delta":"Done!"})),
        sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r2","output":[],"usage":{"input_tokens":20,"output_tokens":10,"total_tokens":30}}})),
    );
    ...
    sq_tx.send(Submission { id: "t1".into(), op: user_turn("run echo") }).await.unwrap();
    sq_tx.send(Submission { id: "s1".into(), op: Op::Shutdown }).await.unwrap();
    codex.run().await.unwrap();
    handle.abort();
    let events = drain(&eq_rx);

    let begin_pos = events.iter().position(|e| matches!(&e.msg, EventMsg::McpToolCallBegin(_)));
    let end_pos = events.iter().position(|e| matches!(&e.msg, EventMsg::McpToolCallEnd(_)));
...
async fn v03_reasoning_then_text() {
    let body = format!(
        "{}{}{}{}",
        sse("response.reasoning_summary_text.delta", serde_json::json!({"type":"response.reasoning_summary_text.delta","delta":"thinking...","summary_index":0})),
        sse("response.reasoning_summary_text.delta", serde_json::json!({"type":"response.reasoning_summary_text.delta","delta":" more","summary_index":0})),
        sse("response.output_text.delta", serde_json::json!({"type":"response.output_text.delta","delta":"Answer: 42"})),
        sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r1","output":[],"usage":{"input_tokens":10,"output_tokens":5,"total_tokens":15}}})),
    );
    let server = MockServer::start(body).await;
    let (sq_tx, eq_rx, codex) = make_codex(&server.base_url());

    sq_tx.send(Submission { id: "t1".into(), op: user_turn("think") }).await.unwrap();
    sq_tx.send(Submission { id: "s1".into(), op: Op::Shutdown }).await.unwrap();
    codex.run().await.unwrap();
    let events = drain(&eq_rx);
    ...
    let reasoning_deltas: Vec<&str> = events.iter().filter_map(|e| match &e.msg {
    ...
    assert_eq!(reasoning_completed.unwrap().summary_text, vec!["thinking... more"]);

    // Agent message events
    assert!(events.iter().any(|e| matches!(&e.msg, EventMsg::ItemStarted(ev) if matches!(&ev.item, tauri_app_lib::protocol::items::TurnItem::AgentMessage(_)))));
    let agent_deltas: Vec<&str> = events.iter().filter_map(|e| match &e.msg {
        EventMsg::AgentMessageContentDelta(d) => Some(d.delta.as_str()),
        _ => None,
    }).collect();
...
async fn v04_empty_response_no_agent_message() {
    let body = sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r1","output":[],"usage":{"input_tokens":5,"output_tokens":0,"total_tokens":5}}}));
    let server = MockServer::start(body).await;
    let (sq_tx, eq_rx, codex) = make_codex(&server.base_url());
...
async fn v05_api_error_emits_error_and_completes() {
    let body = sse("error", serde_json::json!({"type":"error","code":"rate_limit","message":"Too many requests"}));
    let server = MockServer::start(body).await;
    let (sq_tx, eq_rx, codex) = make_codex(&server.base_url());
...
async fn v06_token_usage_emitted() {
    let body = format!(
        "{}{}",
        sse("response.output_text.delta", serde_json::json!({"type":"response.output_text.delta","delta":"hi"})),
        sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r1","output":[],"usage":{"input_tokens":100,"output_tokens":50,"total_tokens":150}}})),
    );
    let server = MockServer::start(body).await;
    let (sq_tx, eq_rx, codex) = make_codex(&server.base_url());
...
async fn v07_no_legacy_events() {
    let body = format!(
        "{}{}",
        sse("response.output_text.delta", serde_json::json!({"type":"response.output_text.delta","delta":"text"})),
        sse("response.completed", serde_json::json!({"type":"response.completed","response":{"id":"r1","output":[],"usage":{"input_tokens":5,"output_tokens":2,"total_tokens":7}}})),
    );
    let server = MockServer::start(body).await;
    let (sq_tx, eq_rx, codex) = make_codex(&server.base_url());
    ...
    assert!(!events.iter().any(|e| matches!(&e.msg, EventMsg::AgentMessageDelta(_))));
    assert!(!events.iter().any(|e| matches!(&e.msg, EventMsg::AgentMessage(_))));
    assert!(!events.iter().any(|e| matches!(&e.msg, EventMsg::UserMessage(_))));
    assert!(!events.iter().any(|e| matches!(&e.msg, EventMsg::AgentReasoningDelta(_))));
}

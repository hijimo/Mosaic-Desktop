...
async fn t01_session_registers_builtin_tools() {
    ...
    let session = Session::new(std::env::current_dir().unwrap(), ConfigLayerStack::new(), tx);
    let router = session.tool_router().await;
    let specs = router.collect_tool_specs();
    assert!(specs.len() >= 5, "expected >= 5 tool specs, got {}", specs.len());
}
...
async fn t02_shell_echo() {
    ...
    let result = router.route_tool_call("shell", serde_json::json!({"command": ["echo", "hello"]})).await;
    match result {
...
async fn t03_list_dir_current() {
    ...
    let result = router.route_tool_call("list_dir", serde_json::json!({"dir_path": cwd})).await;
    match result {
...
async fn t04_specs_include_required_tools() {
    ...
    let names: Vec<String> = specs.iter()
        ...
        .collect();
    for required in &["shell", "apply_patch", "list_dir", "read_file", "grep_files"] {
        assert!(names.iter().any(|n| n == required), "missing '{required}', got: {names:?}");
    }
}
...
async fn t05_apply_patch_invalid() {
    let router = make_real_router();
    let result = router.route_tool_call("apply_patch", serde_json::json!({"input": "not a patch"})).await;
    match result {
...
async fn t06_grep_files_missing_pattern() {
    let router = make_real_router();
    let result = router.route_tool_call("grep_files", serde_json::json!({})).await;
